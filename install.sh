...
do_install() {
  ...
  settings_apply_font_size "$(settings_get PHOENIX_FONT_SIZE)"
...
doctor_de_shortcuts() {
  ...
  local keys='(t|w|left|right|comma|period|return|enter|slash|question)'
  local end="('|\"|,|[[:space:]]|$)"
...
SETTINGS=(
  ...
  "PHOENIX_FONT_SIZE|auto|text|Ghostty font size — 'auto' = 14 on macOS / 12 on Linux, or a number|"
)
...
settings_set() {
  ...
  case "$key" in
    ...
    PHOENIX_FONT_SIZE) settings_apply_font_size "$value" ;;
  esac
...
_resolve_font_size() {
  local v="$1"
  if [[ -z "$v" || "$v" == "auto" ]]; then
    [[ "$(uname -s)" == "Darwin" ]] && echo 14 || echo 12
  else
    echo "$v"
  fi
}

_write_ghostty_user_conf() {
  local mode="$1" color="$2" size="$3"
  run mkdir -p "$(dirname "$PHOENIX_GHOSTTY_USER_CONF")"
  (( DRY_RUN )) && { dry "write $PHOENIX_GHOSTTY_USER_CONF (font=$size mode=$mode)"; return; }
  {
    echo "# Phoenix Term — generated by phoenix-term settings. Do not edit;"
    echo "# change via: phoenix-term settings"
    echo "font-size = $size"
    if [[ "$mode" == "color" ]]; then
...
settings_apply_font_size() {
  local size; size=$(_resolve_font_size "$1")
  _write_ghostty_user_conf "$(settings_get PHOENIX_BG_MODE)" \
                           "$(settings_get PHOENIX_BG_COLOR)" "$size"
  say "ghostty font-size → $size  (reload Ghostty with cmd+, to apply)"
}
...
settings_apply_bg_mode() {
  ...
  _write_ghostty_user_conf "$mode" "$color" \
                           "$(_resolve_font_size "$(settings_get PHOENIX_FONT_SIZE)")"
